...
for l=1:tree_depth
    ...
    % make intervals (ints)
    ints = 1:thd:n+1;
    % build the first block of zeros and random values (Blue/white block in
    % Fig 12a)
...
for i = 3:2:length(idx_tree.breadthfirstiterator)
    ...
    x_idx
    y_idx
    K_true = K(x_idx(1):x_idx(2), y_idx(1):y_idx(2));
    disp(norm(K_true - U_tree.get(idx) * (Z_tree.get(idx))', 'fro') / norm(K_true,'fro'))
    % lower triangle
    x_idx = idx_tree.get(idx_next); y_idx = idx_tree.get(idx);
    x_idx
    y_idx
    K_true = K(x_idx(1):x_idx(2), y_idx(1):y_idx(2));
    disp(norm(K_true - U_tree.get(idx_next) * (Z_tree.get(idx_next))', 'fro') / norm(K_true,'fro'))
end
